...
import org.spacehq.mc.protocol1_7_7.MinecraftProtocol_177;
...
import eu.matejkormuth.staticmc.StaticMC;
...
public class ServerListener extends SessionAdapter {
                ...
                switch (packet.getIntent()) {
                    ...
                    case LOGIN:
                        ...
                        if (packet.getProtocolVersion() > ProtocolConstants.PROTOCOL_VERSION) {
                            ...
                            // Try to switch to 1.7.4
                            if (packet.getProtocolVersion() == org.spacehq.mc.protocol1_7_7.ProtocolConstants.PROTOCOL_VERSION) {
                                StaticMC.getLogger().info(
                                        "Trying to inject different protocol (1.7.7) for client "
                                                + event.getSession().getHost() + ":"
                                                + event.getSession().getPort());
                                
                                try {
                                    Constructor<MinecraftProtocol_177> c = MinecraftProtocol_177.class.getDeclaredConstructor(new Class[0]);
                                    if (!c.isAccessible()) {
                                        c.setAccessible(true);
                                    }
                                    MinecraftProtocol_177 oldProtocol = c.newInstance(new Object[0]);
                                    
                                    oldProtocol.setMode(
                                            eu.matejkormuth.staticmc.ProtocolMode.LOGIN,
                                            false, event.getSession());
                                    
                                    Field f = TcpSession.class.getDeclaredField("protocol");
                                    if (!f.isAccessible()) {
                                        f.setAccessible(true);
                                    }
                                    f.set(event.getSession(), oldProtocol);
                                    ...
                                    e.printStackTrace();
                                    event.getSession().disconnect(
                                            "Sorry, switching to 1.7.7 protocol failed. Please use version "
                                                    + ProtocolConstants.GAME_VERSION
                                                    + ".");
                                }
                            }
            ...
            if (event.getPacket() instanceof LoginStartPacket) {
                ...
                if (verify) {
                    ...
                    GameProfile profile = new GameProfile(
                            ...
                            this.username);
                    int threshold = event.getSession().getFlag(
                            ProtocolConstants.SERVER_COMPRESSION_THRESHOLD);
    ...
    private class UserAuthThread extends Thread {
        ...
        @Override
        public void run() {
            MinecraftProtocol_18 protocol = (MinecraftProtocol_18) this.session.getPacketProtocol();
            try {
